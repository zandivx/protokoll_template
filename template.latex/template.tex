...
\usepackage{amsmath, amssymb, amstext, mathtools}  % math packages (American Math Society) + Verbesserung von amsmath (mathtools - amsmath eigentlich nicht nötig)
...
\usepackage{tabularray}  % best table ever
\usepackage{xcolor}  % custom colors
%\usepackage{diagbox}  % diagonal boxes in tables  NOT WORKING RN WITH TABULARRAY!
...
% text
\usepackage[locale=DE, uncertainty-mode=separate]{siunitx}  % bessere Darstellung von Einheiten - \pm soll \pm bleiben und nicht ..(.)
...
\cfoot{\pagemark{} / \pageref*{LastPage}}  % Fuß Mitte - * ref but no hyperlink
% {}: empty statement
% \ : protected space
% \,: small space
...
\begin{table}[H]   % GENAU hier (package float)
    \centering
    \begin{samepage}  % caption and table on same page
        ...
        \begin{tblr}{
            ...
            vlines,
            %width=0.8\linewidth,
            columns={c,m},  % center, middle as base case if not specified differently
            colspec={*{4}{X[c,m]}},  % multiplication syntax: *{n}{thing}
            cell{2}{2}={r=2}{c,m},  % like \SetCell
